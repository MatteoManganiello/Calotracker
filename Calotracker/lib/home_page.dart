...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadAttivitaScelteOggi() async {
    ...
    try {
      ...
      if (!mounted) return;
      parsed.sort((a, b) {
        final at = DateTime.tryParse(a['timestamp']?.toString() ?? '')?.millisecondsSinceEpoch ?? 0;
        final bt = DateTime.tryParse(b['timestamp']?.toString() ?? '')?.millisecondsSinceEpoch ?? 0;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (durataStr.isNotEmpty)
                        Text('Durata consigliata: $durataStr',
                ...
                children: [
                  Expanded(
                    child: _ColoredCard(
                      ...
                      insetWhite: true,
                      iconTopLeft: true,
                      trailing: _buildCalorieTrailing(const Color.fromARGB(255, 211, 66, 66)),
                      onTap: () async {
                        await Navigator.push(
                          context,
                          MaterialPageRoute(builder: (_) => const DailyCaloPage()),
                        );
      ...
      child: Row(
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('timer', style: TextStyle(fontSize: 19)),
          ),
        ],
      ),
    );
        ...
        child: insetWhite
            ? Padding(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: insetColor,
                    borderRadius: innerBorderRadius ?? BorderRadius.circular(insetRadius),
                  ),
                  child: content,
                ),
              )
