...
// scanner/servizi (adegua il path se necessario al tuo progetto)
...
/// ---- PALETTE ROSSA ----
const Color _brandRed = Color.fromARGB(255, 211, 66, 66);   // colore principale
const Color _pageBgRed = Color.fromARGB(255, 244, 194, 194); // sfondo pagina (lasciato come da te)
const Color _cardBorderRed = Color.fromARGB(255, 172, 58, 58); // bordo card

/// Pagina per inserire e gestire le calorie giornaliere.
...
class _DailyCaloPageState extends State<DailyCaloPage> {
  // Prefs keys
  static const String _entriesKeyPrefix = 'calo_entries_'; // + yyyy-MM-dd
  ...
  bool _loading = true;
  int? _dailyGoal; // obiettivo (se esiste)
  ...
  // Apre lo scanner e precompila la bottom sheet con i dati trovati
  Future<void> _onScanPressed() async {
    ...
    // accetta solo EAN/UPC plausibili (8–14 cifre)
    final isBarcode = RegExp(r'^\d{8,14}$').hasMatch(barcode);
    if (!isBarcode) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Codice non valido: non sembra un EAN/UPC')),
      );
      return;
    }
    ...
    if (online) {
      try {
        ...
        // ignora, fallback con dati minimi sotto
      }
    }
  ...
  // Esteso per accettare valori iniziali (oltre alla modalità "existing")
                    ...
                    onPressed: () {
                      ...
                      setState(() {
                        if (existing == null) {
                          ...
                          final i =
                              _entries.indexWhere((e) => e.id == existing.id);
  ...
  Future<void> _confirmClearToday() async {
    ...
    if (ok == true) {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Voci di oggi rimosse')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    final progress =
        (_dailyGoal != null && _dailyGoal! > 0) ? (_totalKcal / _dailyGoal!) : null;

    return Scaffold(
      // ✅ sfondo pagina (lasciato come da te)
      ...
      // ✅ due pulsanti galleggianti: più centrati, senza ombra, rosso brand
      ...
      body: _loading
          ? const Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation(_brandRed)))
          : RefreshIndicator(
              onRefresh: _loadAll,
              color: _brandRed,
              child: ListView(
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            // 🔁 Icona cambiata: PRIMA Icons.restaurant_outlined, ORA Icons.fastfood
      ...
      child: Card(
        ...
        child: ListTile(
          ...
          subtitle: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Calorie/unità: ${entry.caloriesPerUnit} • Quantità: ${entry.quantity}'),
              if (entry.notes != null && entry.notes!.isNotEmpty) Text(entry.notes!),
            ],
          ),
          trailing: Column(
